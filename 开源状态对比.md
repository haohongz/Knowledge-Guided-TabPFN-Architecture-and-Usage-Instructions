...
> **nanotabicl 是什么？** TabICL 完整代码有几千行，读起来费劲。nanotabicl 是 INRIA 团队自己写的极简版，把核心逻辑压缩到只有 170 行 Python，去掉了所有工程细节（日志、分布式训练、配置文件等），只保留最核心的：怎么生成合成数据（prior）、怎么训练、怎么推理。想理解 TabICL 怎么工作，看这 170 行就够了，后面改 prior 也是在这个基础上改最方便。
